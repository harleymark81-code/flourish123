...
async def send_email(to: str, subject: str, html: str) -> bool:
    ...
    try:
        ...
        if result:
            ...
            logger.warning(f"[email] send returned no ID for '{subject}' → {to}")
        return result
        ...
        <table width="100%" style="max-width:560px;background:{WHITE};border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(83,74,183,0.08);">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,{PURPLE},{PURPLE_DARK});padding:32px 40px;text-align:center;">
              <p style="margin:0;font-size:28px;font-weight:800;color:{WHITE};letter-spacing:-0.5px;">🌸 Flourish</p>
...
def _btn(text: str, url: str, colour: str = PURPLE) -> str:
    return f"""<a href="{url}" style="display:inline-block;background:{colour};color:{WHITE};text-decoration:none;font-weight:700;font-size:15px;padding:14px 28px;border-radius:10px;margin-top:24px;">{text}</a>"""


def _h1(text: str) -> str:
    return f'<h1 style="margin:0 0 12px;font-size:24px;font-weight:800;color:{TEXT};letter-spacing:-0.4px;line-height:1.3;">{text}</h1>'


def _p(text: str, muted: bool = False) -> str:
    colour = TEXT_MUTED if muted else TEXT
    return f'<p style="margin:0 0 14px;font-size:15px;color:{colour};line-height:1.65;">{text}</p>'


def _divider() -> str:
    return f'<hr style="border:none;border-top:1px solid #EEEDF8;margin:24px 0;" />'


def _highlight_box(html: str) -> str:
    return f'<div style="background:{BG};border-left:4px solid {PURPLE};border-radius:0 8px 8px 0;padding:16px 20px;margin:20px 0;">{html}</div>'
...
async def send_welcome_email(to: str, name: str) -> bool:
    ...
    body = f"""
      {_h1(f"Welcome to Flourish, {first} 🌸")}
      ...
      {_highlight_box(f"""
        <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:{PURPLE};text-transform:uppercase;letter-spacing:0.8px;">What you can do right now</p>
        <p style="margin:0;font-size:14px;color:{TEXT};line-height:1.6;">
          ✅ &nbsp;Scan any food and get your personalised score<br/>
          ✅ &nbsp;See how it scores on naturalness<br/>
...
async def send_subscription_confirmed_email(to: str, name: str, plan: str) -> bool:
    ...
    body = f"""
      ...
      {_p(f"Hi {first}, your subscription is confirmed and all premium features are unlocked.")}
      {_highlight_box(f"""
...
# ── 3. Trial ending reminder (sent ~24h before trial expires) ─────────────────

async def send_trial_ending_email(to: str, name: str) -> bool:
    first = name.split()[0] if name else "there"
    body = f"""
      {_h1("Your free trial ends tomorrow ⏰")}
      {_p(f"Hi {first}, just a quick heads-up — your 3-day Flourish Premium trial ends in 24 hours.")}
      ...
      {_highlight_box(f"""
        <p style="margin:0;font-size:14px;color:{TEXT};line-height:1.6;">
          <strong>No action needed if you want to keep Premium.</strong> Your subscription will continue automatically and you won't be charged until after the trial ends.
        </p>
      """)}
      {_p("Want to cancel? You can do it in one tap from the Profile tab before your trial ends — no questions asked.", muted=True)}
      {_btn("Manage My Subscription", f"{FRONTEND_URL}/#profile")}
    """
...
async def send_scan_limit_email(to: str, name: str) -> bool:
    ...
    body = f"""
      ...
      {_p(f"Hi {first}, you've hit your 3 free scans for today. Come back tomorrow for 3 more — or upgrade to Premium for unlimited scans every day.")}
      {_highlight_box(f"""
        <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:{PURPLE};text-transform:uppercase;letter-spacing:0.8px;">With Flourish Premium</p>
        <p style="margin:0;font-size:14px;color:{TEXT};line-height:1.6;">
          🔓 &nbsp;Unlimited food scans, every day<br/>
          🔓 &nbsp;Hormonal impact score for every food<br/>
          🔓 &nbsp;Food diary, symptom tracking &amp; insights<br/>
          🔓 &nbsp;Personalised meal planner
        </p>
      """)}
      {_p(f"Start your <strong>3-day free trial</strong> today — no charge until day 4, cancel anytime.")}
...
async def send_referral_reward_email(to: str, referrer_name: str, referred_name: str) -> bool:
    ...
    body = f"""
      ...
      {_p(f"Great news, {first} — {referred_first} just subscribed to Flourish Premium using your referral link.")}
      {_highlight_box(f"""
      ...
      {_btn("View My Referrals", f"{FRONTEND_URL}/#profile")}
    """
    return await send_email(to, f"🎉 {referred_first} subscribed — you've earned a free month!", _wrap(body))


# ── 6. Password reset (migrated from EmailJS) ─────────────────────────────────

async def send_password_reset_email(to: str, name: str, reset_link: str) -> bool:
    first = name.split()[0] if name else "there"
    body = f"""
      {_h1("Reset your Flourish password")}
      {_p(f"Hi {first}, we received a request to reset your password.")}
...
# ── 7. Weekly report (migrated from EmailJS) ──────────────────────────────────
...
async def send_weekly_report_email(
    ...
    green_list = (
        "".join(f'<li style="color:{GREEN};font-weight:600;">{f}</li>' for f in green_foods)
        if green_foods else f'<li style="color:{TEXT_MUTED};">Keep logging to see your top foods</li>'
    )
    red_list = (
        "".join(f'<li style="color:#A32D2D;">{f}</li>' for f in red_foods)
        if red_foods else f'<li style="color:{GREEN};font-weight:600;">None this week — great work! 🎉</li>'
    )

    body = f"""
      {_h1(f"Your Flourish weekly report, {first}")}
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;">
        <tr>
          <td width="48%" style="background:{BG};border-radius:12px;padding:20px;text-align:center;">
            ...
            <p style="margin:0;font-size:36px;font-weight:800;color:{PURPLE};">{scan_count}</p>
          </td>
          <td width="4%"></td>
          <td width="48%" style="background:{BG};border-radius:12px;padding:20px;text-align:center;">
            ...
            <p style="margin:0;font-size:36px;font-weight:800;color:{score_colour};">{avg_score}</p>
          </td>
        </tr>
      </table>
    ...
    """
    return await send_email(to, f"Your Flourish weekly food report 📊", _wrap(body))
