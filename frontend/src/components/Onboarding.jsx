...
const VALIDATION = {
  pcos: { emoji: "💜", headline: "Living with PCOS is harder than most people know.", body: "Your body processes food differently to most people. What works for others — the standard diets, the generic advice — often doesn't work for you. That's not a failure. That's just PCOS. Flourish is built to understand that." },
  endometriosis: { emoji: "🌸", headline: "Endometriosis affects everything — including what you eat.", body: "Certain foods fuel inflammation and make your symptoms significantly worse. But nobody ever tells you which ones. Flourish rates every food specifically for endometriosis, so you can finally eat with confidence." },
  thyroid: { emoji: "⚡", headline: "Your thyroid is sensitive to what you eat in ways most people never realise.", body: "Foods that are perfectly healthy for others can suppress your thyroid function or interfere with your medication. Flourish tracks this so you don't have to figure it out alone." },
  ibs: { emoji: "🌿", headline: "IBS makes every meal feel like a gamble.", body: "You've probably spent years trying to figure out your triggers — the anxiety before eating, the unpredictability. Flourish scores every food for gut sensitivity so you can eat without the fear of not knowing." },
  autoimmune: { emoji: "🛡️", headline: "Your immune system is working overtime, and food matters more than you think.", body: "Certain foods trigger inflammatory responses that make autoimmune conditions significantly worse. It's different for everyone — but Flourish personalises every rating to your exact condition." },
  hormonal_imbalance: { emoji: "⚖️", headline: "Hormonal imbalance affects every system in your body.", body: "Food directly impacts oestrogen, cortisol, and insulin. What you eat either supports your hormones or disrupts them. Flourish makes that visible for the first time." },
  not_sure: { emoji: "💛", headline: "Your body is trying to tell you something.", body: "You don't need a diagnosis to deserve personalised food intelligence. Flourish is built for everyone navigating unexplained symptoms, chronic fatigue, and a body that doesn't feel quite right." },
  other: { emoji: "✨", headline: "Your health journey is your own.", body: "Flourish adapts to your unique situation. The more we know about your body, the more precise and helpful your food ratings become — starting right now." },
};
...
export default function Onboarding({ onComplete }) {
  ...
  // Progress bar: screens 2–14 → 0% to 100%
        ...
        <AnimatePresence mode="wait" custom={dir}>
          ...
          {screen === 11 && (
            <motion.div key="s11" custom={dir} variants={variants} initial="initial" animate="animate" exit="exit">
              ...
              <Btn onClick={() => goTo(12)} style={{ marginTop: 12 }}>See everything Flourish does →</Btn>
            </motion.div>
          )}
            ...
            <motion.div key="s14" custom={dir} variants={variants} initial="initial" animate="animate" exit="exit">
              ...
              {insights.map((insight, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0, transition: { delay: i * 0.12 } }}
                  ...
                  <div style={{ width: 22, height: 22, borderRadius: "50%", background: PRI, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
                    <Star size={10} color="#fff" fill="#fff" />
                  </div>
                  <p style={{ fontSize: 14, color: P, margin: 0, lineHeight: 1.5 }}>{insight}</p>
                </motion.div>
              ))}
              <Btn onClick={() => goTo(15)} style={{ marginTop: 10 }}>See it in action →</Btn>
            </motion.div>
          )}
          ...
          {screen === 15 && (
            <motion.div key="s15" custom={dir} variants={variants} initial="initial" animate="animate" exit="exit">
              <div style={{ textAlign: "center", marginBottom: 28 }}>
                <div style={{ width: 56, height: 56, borderRadius: 16, background: `linear-gradient(135deg, ${PRI}, #756AD9)`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 28, boxShadow: "0 4px 20px rgba(83,74,183,0.3)" }}>🔍</div>
                ...
                <p style={{ fontSize: 15, color: S, lineHeight: 1.55 }}>Rate any food right now. Full premium experience — nothing locked.</p>
              </div>
              <input value={scanFood} onChange={e => setScanFood(e.target.value)} onKeyDown={e => e.key === "Enter" && handleScan()}
              ...
              {scanError && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ fontSize: 14, color: "#A32D2D", background: "rgba(163,45,45,0.08)", borderRadius: 10, padding: "10px 14px", margin: "8px 0" }}>{scanError}</motion.p>}
              <Btn onClick={handleScan} disabled={!scanFood.trim()} loading={scanLoading}>
                {scanLoading ? "Analysing your food..." : `Rate "${scanFood || "your food"}" →`}
              </Btn>
              <button onClick={() => onComplete()} style={{ width: "100%", background: "none", border: "none", color: "var(--text-muted)", fontSize: 13, cursor: "pointer", padding: "16px 0 0", fontWeight: 500 }}>
                Skip for now
              </button>
            </motion.div>
          )}
            ...
            <motion.div key="s16" custom={dir} variants={variants} initial="initial" animate="animate" exit="exit">
              ...
              {scanResult.forYourCondition && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.4 } }}
                  ...
                  <p style={{ fontSize: 11, fontWeight: 700, color: PRI, textTransform: "uppercase", letterSpacing: 0.8, margin: "0 0 5px" }}>For your condition</p>
              ...
              {scanResult.flags && (
                <div style={{ marginBottom: 14 }}>
                  {scanResult.flags.warnings?.slice(0, 3).map((w, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 6 }}><span style={{ fontSize: 14 }}>⚠️</span><p style={{ fontSize: 13, color: S, margin: 0, lineHeight: 1.4 }}>{w}</p></div>))}
                  {scanResult.flags.positives?.slice(0, 3).map((pos, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 6 }}><span style={{ fontSize: 14 }}>✅</span><p style={{ fontSize: 13, color: S, margin: 0, lineHeight: 1.4 }}>{pos}</p></div>))}
                </div>
              )}
              ...
              <Btn onClick={() => goTo(17)}>See your options →</Btn>
            </motion.div>
          )}

          {/* ── 17 · Post-scan paywall ───────────────────────────────────────── */}
          {screen === 17 && (
            <motion.div key="s17" custom={dir} variants={variants} initial="initial" animate="animate" exit="exit">
              <div style={{ textAlign: "center", marginBottom: 28 }}>
              ...
              {/* Annual */}
              <motion.div whileTap={{ scale: 0.98 }} onClick={() => !planLoading && handlePlan("annual")}
                style={{ background: `linear-gradient(135deg, ${PRI}, #756AD9)`, borderRadius: 18, padding: "20px 22px", marginBottom: 12, position: "relative", overflow: "hidden", boxShadow: "0 6px 28px rgba(83,74,183,0.35)", cursor: "pointer" }}>
                ...
                <div style={{ background: "#fff", borderRadius: 12, padding: "13px 0", textAlign: "center" }}>
                  <p style={{ color: PRI, fontWeight: 800, fontSize: 15, margin: 0 }}>
              ...
              {/* Monthly */}
              <motion.div whileTap={{ scale: 0.98 }} onClick={() => !planLoading && handlePlan("monthly")}
                ...
                <div style={{ background: "var(--bg-elevated)", borderRadius: 12, padding: "12px 0", textAlign: "center", border: "1px solid var(--border)" }}>
                  <p style={{ color: PRI, fontWeight: 700, fontSize: 14, margin: 0 }}>
                    {planLoading === "monthly" ? "Creating your trial..." : "Start 3-Day Free Trial →"}
                  </p>
                </div>
              </motion.div>

              {planError && <p style={{ fontSize: 14, color: "#A32D2D", background: "rgba(163,45,45,0.08)", borderRadius: 10, padding: "10px 14px", marginBottom: 12 }}>{planError}</p>}
              <p style={{ fontSize: 12, color: "var(--text-muted)", textAlign: "center", lineHeight: 1.6, marginBottom: 12 }}>Cancel anytime during your trial — you won't be charged a penny.</p>
              <button onClick={() => onComplete()} style={{ width: "100%", background: "none", border: "none", color: "var(--text-muted)", fontSize: 14, cursor: "pointer", padding: "8px 0 0", fontWeight: 500 }}>
                Maybe later
              </button>
            </motion.div>
          )}
