  ...
  return (
    <motion.div
      ...
      <motion.div
        ...
        exit={{ y: "100%" }}
        style={{ width: "100%", maxWidth: 480, margin: "0 auto", background: "#fff", borderRadius: "24px 24px 0 0", padding: "24px 20px 48px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1A1A24", margin: 0 }}>How are you feeling today?</h2>
          <motion.button whileTap={{ scale: 0.9 }} onClick={onClose} style={{ background: "#F8F7FF", border: "1px solid #E8E6FF", borderRadius: 10, width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            <X size={16} color="#6B6A7C" />
          </motion.button>
        </div>
        <p style={{ fontSize: 14, color: "#6B6A7C", margin: "0 0 24px" }}>Takes under 10 seconds. Rate each from 1-5.</p>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {[1, 2, 3, 4, 5].map(val => (
                  <motion.button
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: scores[item.key] >= val ? "#fff" : "#A09FAD" }}>{val}</span>
                  </motion.button>
                ))}
              </div>
            </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          onClick={handleSave}
          style={{ width: "100%", background: saved ? "#639922" : "#534AB7", color: "#fff", border: "none", borderRadius: 12, padding: "16px", fontSize: 16, fontWeight: 600, cursor: "pointer", marginTop: 24, transition: "background 0.3s" }}>
